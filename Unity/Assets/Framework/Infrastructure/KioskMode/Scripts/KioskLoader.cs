...
namespace GSFC.ARVR.MRET.Infrastructure
{
    ...
    public class KioskLoader : MRETBehaviour
    {
        ...
        protected override void MRETStart()
        {
            ...
            if (modeNavigator == null)
            {
                Debug.LogError("[" + ClassName + "] Invalid state encountered. Unable to locate " + nameof(ModeNavigator) + "...");
            }
        }
        ...
        private void LoadScene(string scene)
        {
            ...
            {
                Debug.LogWarning("[" + ClassName + "->" + nameof(LoadScene) + "] Unable to load project: '" + scene + "'");
            }
        }
        ...
        private void SetOtherAttributes()
        {
            foreach (XmlNode attr in xmlOtherAttributes[0].ChildNodes)
            {
                ...
                {
                    case "ObjectPanels":
                        Framework.MRET.PartPanelEnabled = bool.Parse(attr.InnerXml.ToString());
                        break;

                    default:
                        Debug.LogWarning("[" + ClassName + "->" + nameof(SetOtherAttributes) + "] Unknown Other Attribute " + attr.Name);
                        break;
                }
            }
        }
