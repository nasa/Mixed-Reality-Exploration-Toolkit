...
namespace GSFC.ARVR.MRET.Components.LineDrawing
{
    ...
    /// </summary>
    public class VolumetricDrawing : LineDrawing
    {
        /// <summary>
        ...
        public override float length
        {
            get
            {
                if (_points == null)
                {
                    Debug.LogError("[VolumetricDrawing->length] Not initialized.");
                    return 0;
                }
        ...
        public static new VolumetricDrawing Create(string name, float diameter, Color32 color, Vector3[] positions, Guid? uuid = null, float cutoff = -1, bool showMeasurement = false)
        {
        ...
        public override void Initialize(Guid uuid)
        {
            ...
            segmentPrefab = Infrastructure.Framework.MRET.SceneObjectManager.volumetricDrawingSegmentPrefab;
            cornerPrefab = Infrastructure.Framework.MRET.SceneObjectManager.volumetricDrawingCornerPrefab;
        ...
        public override void RemovePoint(int index)
        {
            ...
            {
                Debug.LogError("[VolumetricDrawing->RemovePoint] Invalid index.");
                return;
            }
        ...
        public override void ReplacePoint(int index, Vector3 value)
        {
            if (index >= _points.Count || index < 0)
            {
                Debug.LogError("[VolumetricDrawing->ReplacePoint] Invalid index.");
                return;
            }
        ...
        private void RecolorLine()
        {
            foreach (MeshRenderer rend in meshContainer.GetComponentsInChildren<MeshRenderer>())
            {
                foreach (Material mat in rend.materials)
                {
                    ...
                    {
                        Debug.LogError("[VolumetricDrawing->RecolorLine] Incorrect shader. Must be HDRP/Lit");
                        continue;
                    }
        ...
        private void RecolorSegment(GameObject segment)
        {
            foreach (MeshRenderer rend in segment.GetComponentsInChildren<MeshRenderer>())
            {
                foreach (Material mat in rend.materials)
                {
                    if (mat.shader.name != "HDRP/Lit")
                    {
                        Debug.LogError("[VolumetricDrawing->RecolorSegment] Incorrect shader. Must be HDRP/Lit.");
                        continue;
                    }
