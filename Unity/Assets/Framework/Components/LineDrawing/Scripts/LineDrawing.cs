...
namespace GSFC.ARVR.MRET.Components.LineDrawing
{
    ...
    /// </summary>
    public class LineDrawing : CustomShape.CustomShape
    {
        /// <summary>
        ...
        public virtual Vector3[] points
        {
            get
            {
                if (lineRenderer == null)
                {
                    Debug.LogError("[LineDrawing->points] Not initialized.");
                    return null;
                }
        ...
        /// </summary>
        public DrawingEditController drawingEditController { get; private set; }
        ...
        public static LineDrawing Create(string name, float width, Color32 color, Vector3[] positions, Guid? uuid = null, float cutoff = -1, bool showMeasurement = false)
        {
            ...
            lineDrawing.drawingEditController.SetUnits(LineDrawingManager.Units.meters);
        ...
        public virtual void Initialize(Guid uuid)
        {
            ...
            this.uuid = uuid;
            lineRenderer.material = Infrastructure.Framework.MRET.LineDrawingMaterial;
        ...
        public virtual void SetWidth(float width)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->SetWidth] Not initialized.");
                return;
            }
        ...
        public virtual float GetWidth()
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->GetWidth] Not initialized.");
                return 0;
            }
        ...
        public virtual void SetColor(Color32 color)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->SetColor] Not initialized.");
                return;
            }

            if (lineRenderer.material == null)
            {
                Debug.LogError("[LineDrawing->SetColor] No material.");
                return;
            }

            if (lineRenderer.material.shader.name != "HDRP/Lit")
            {
                Debug.LogError("[LineDrawing->SetColor] Incorrect shader. Must be HDRP/Lit.");
                return;
            }
        ...
        public virtual Color32 GetColor()
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->GetColor] Not initialized.");
                return Color.gray;
            }

            if (lineRenderer.material == null)
            {
                Debug.LogError("[LineDrawing->GetColor] No material.");
                return Color.gray;
            }

            if (lineRenderer.material.shader.name != "HDRP/Lit")
            {
                Debug.LogError("[LineDrawing->GetColor] Incorrect shader. Must be HDRP/Lit.");
                return Color.gray;
            }
        ...
        public virtual void AddPoint(Vector3 point, int index)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->AddPoint] Not initialized.");
                return;
            }
            ...
            {
                Debug.LogError("[LineDrawing->AddPoint] Invalid index.");
                return;
            }
        ...
        public virtual void AddPoints(Vector3[] points, int startIndex)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->AddPoints] Not initialized.");
                return;
            }
            ...
            {
                Debug.LogError("[LineDrawing->AddPoints] Invalid index.");
                return;
            }
        ...
        public virtual void RemovePoint(int index)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->RemovePoint] Not initialized.");
                return;
            }
            ...
            if (index >= lineRenderer.positionCount || index < 0)
            {
                Debug.LogError("[LineDrawing->RemovePoint] Invalid index.");
                return;
            }
        ...
        public virtual void ReplacePoint(int index, Vector3 value)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->ReplacePoint] Not initialized.");
                return;
            }

            if (index >= lineRenderer.positionCount || index < 0)
            {
                Debug.LogError("[LineDrawing->ReplacePoint] Invalid index.");
                return;
            }
        ...
        public void AppendLine(LineDrawing lineToAppend, bool appendBackwards, bool appendToFront)
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->AppendLine] Not initialized.");
                return;
            }
        ...
        public void SetMeasurementUnits(LineDrawingManager.Units units)
        {
        ...
        public void EnableEndpointVisuals()
        {
            ...
            {
                Debug.LogError("[LineDrawing->EnableEndpointVisuals] No points.");
                return;
            }
        ...
        public void EnableMiddlePointVisuals(int numberOfPoints)
        {
            ...
            {
                Debug.LogWarning("[LineDrawing->EnableMiddlePointVisuals] No middle points.");
                return;
            }
            if (numberOfPoints > linePoints.Length - 2)
            {
                Debug.LogWarning("[LineDrawing->EnableMiddlePointVisuals] Desired number of points exceeds actual.");
        ...
        protected void SetUpMeasurementText()
        {
            ...
            measurementText = measurementObj.AddComponent<MeasurementText>();
        ...
        {
            GameObject visual = Instantiate(Infrastructure.Framework.MRET.LineDrawingManager.drawingEndVisualPrefab);
            ...
            {
                Debug.LogError("[LineDrawing->SetUpEndPoint] Error loading point visual.");
                return null;
            }
            ...
            Action<Vector3, InputHand> pointAppendAction = (Vector3 newPoint, InputHand hand) => {
                ...
                if (ldc == null)
                {
                    Debug.LogError("[LineDrawing->EndPointChangeAction] No drawing edit controller.");
                }
            ...
            Action<Vector3, InputHand, DrawingPointVisual> pointAppendEndAction =
                ...
                if (ldc == null)
                {
                    Debug.LogError("[LineDrawing->EndPointChangeEndAction] No drawing edit controller.");
                }
                else
                {
                    ldc.FinishLine();
                    if (touchingVisual != null && touchingVisual.drawing != this)
                    {
                        ...
                        Infrastructure.Framework.MRET.SceneObjectManager.DestroySceneObject(touchingVisual.drawing.uuid);
                    }
        ...
        {
            GameObject visual = Instantiate(Infrastructure.Framework.MRET.LineDrawingManager.drawingPointVisualPrefab);
            ...
            if (drawingPointVisual == null)
            {
                Debug.LogError("[LineDrawing->SetUpMiddlePoint] Error loading point visual.");
                return null;
            }
            Action<Vector3, InputHand> pointChangeAction = (Vector3 newPoint, InputHand hand) => {
                /*int i = index - 1;
        ...
        {
            GameObject drawingEditObject = Instantiate(Infrastructure.Framework.MRET.LineDrawingManager.drawingEditPrefab);
            ...
            drawingEditController = drawingEditObject.GetComponent<DrawingEditController>();
            if (drawingEditController == null)
            {
                Debug.LogError("[LineDrawing->SetUpEditController] Unable to set up.");
                return;
            }
        ...
        private Mesh GetMesh()
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->GetMesh] Not initialized.");
                return null;
            }
        ...
        private void SetUpMeshCollider()
        {
            if (lineRenderer == null)
            {
                Debug.LogError("[LineDrawing->SetUpMeshCollider] Not initialized.");
                return;
            }
            ...
            if (mesh == null)
            {
                Debug.LogError("[LineDrawing->SetUp MeshCollider] Failed to get mesh.");
                return;
            }
