...
public class Note : MonoBehaviour, ISelectable
{
    ...
    public GameObject fullNote, minimizedNote, drawingContainer, titleKeyboard, informationKeyboard;
    ...
    private SynchronizedNote synchronizedNote;
    ...
    void Start()
    {
        unityProject = GameObject.Find("LoadedProject").GetComponent<UnityProject>();
        ...
        {
            synchronizedNote = gameObject.AddComponent<SynchronizedNote>();
        }
        ...
        titleText.SetKeyboard(titleKeyboard);
        informationText.SetKeyboard(informationKeyboard);
    }
    ...
    public void StopDrawing(GameObject controller)
    {
        ...
        if (cEvents)
        {
            if (cEvents.gameObject == VRTK.VRTK_DeviceFinder.GetControllerRightHand() && currentRightLine)
            {   // Right.
                Vector3[] points = new Vector3[currentRightLine.positionCount];
                currentRightLine.GetPositions(points);
                undoManager.AddAction(ProjectAction.AddNoteDrawingAction(transform.name, currentRightLine.name, points),
                    ProjectAction.DeleteNoteDrawingAction(transform.name, currentRightLine.name));
                currentRightLine = null;
                drawingRight = false;
                rightUsingController = null;
                rightRaycaster = null;
            }
            else if (cEvents.gameObject == VRTK.VRTK_DeviceFinder.GetControllerLeftHand() && currentLeftLine)
            {   // Left.
                Vector3[] points = new Vector3[currentLeftLine.positionCount];
                currentLeftLine.GetPositions(points);
                undoManager.AddAction(ProjectAction.AddNoteDrawingAction(transform.name, currentLeftLine.name, points),
                    ProjectAction.DeleteNoteDrawingAction(transform.name, currentLeftLine.name));
    ...
    public void StartDrawing(GameObject controller)
    {
        ...
        if (cEvents)
        {
            if (cEvents.gameObject == VRTK.VRTK_DeviceFinder.GetControllerRightHand())
            {   // Right.
                rightRaycaster = cEvents.GetComponent<ControllerUIRaycastDetectionManager>();
                drawingRight = true;
                rightUsingController = controller;
            }
            else if (cEvents.gameObject == VRTK.VRTK_DeviceFinder.GetControllerLeftHand())
            {   // Left.
                leftRaycaster = cEvents.GetComponent<ControllerUIRaycastDetectionManager>();
                drawingLeft = true;
    ...
    public static Note MakeNote(GameObject notePrefab, Vector3 pos, Quaternion rot, int id)
	{
    ...
    public static Note MakeNote(GameObject notePrefab, Vector3 pos, Quaternion rot, string noteName)
    {
        GameObject noteObj = Instantiate(notePrefab, pos, rot, GameObject.Find("Notes").transform);
	...
	public static NoteData toSerializable(Note note)
	{
        return new NoteData()
        {
	...
	public static Note fromSerializable(NoteData data, NoteDrawingType[] drawings, int index, Guid guid)
	{
		Note n = MakeNote(NoteManager.notePrefab, data.pos, data.rot, index);
        n.guid = guid;
    ...
    public static Note fromSerializable(NoteData data, NoteDrawingType[] drawings, string noteName, Guid guid)
    {
        Note n = MakeNote(NoteManager.notePrefab, data.pos, data.rot, noteName);
        n.guid = guid;
        n.titleText.text = data.title;
        n.informationText.text = data.information;
        n.DeserializeDrawings(drawings);
    ...
    private void CaptureTitleChange()
    {
        ...
        {
            undoManager.AddAction(ProjectAction.ChangeNoteTextAction(transform.name, titleText.text, informationText.text),
                ProjectAction.ChangeNoteTextAction(transform.name, lastRecordedTitle, lastRecordedInformation));
...
#region ScaleManagement
    ...
    private UnityProject unityProject = null;
