...
namespace Assets.VDE.Layouts.NetworkTopologyTD
{
    /// <summary>
    /// this is an example on how to add custom Layouts that would be activated with Activator.CreateInstance
    /// </summary>
    public class Layout : Assets.VDE.Layouts.Layout
    {
        Log log;
        List<Container> shapesInRow = new List<Container> { };
        /*
        ...
        */
        ...
        override internal Vector3 GetTier1ContainerPosition(Container container, float distance)
        {
            return new Vector3(container.entity.pos * distance, 0, 0);
            /*
            ...
            return new Vector3(Mathf.Cos(anger), 0, Mathf.Sin(anger)) * distance / 2;
            */
        }
        /// <summary>
        ...
        override internal System.Collections.IEnumerator Reinitialize()
        {
            ...
            foreach (Container shallowGroup in shapesInRow)
            {
        ...
        {
            Transform trafoGroot = GetGroot().containers.GetCurrentGroup().transform;//.localEulerAngles = 
            trafoGroot.localEulerAngles = new Vector3(0, 0, 180);
            trafoGroot.localPosition = new Vector3(50, 0, 0);

            Entity unknown = data.entities.Get("Unknown");
            if (!(unknown is null))
            {
                Transform trafoUnk = unknown.containers.GetCurrentGroup().transform;
                if (!(trafoUnk is null))
                {
                    trafoUnk.localEulerAngles = new Vector3(0, 0, 0);
                    trafoUnk.localPosition = new Vector3(90, -60, 80);
                }
            }

            Entity susped = data.entities.Get("Suspected entities from threat feeds");
            if (!(susped is null))
            {
                Transform trafoUnk = susped.containers.GetCurrentGroup().transform;
                if (!(trafoUnk is null))
                {
                    trafoUnk.localEulerAngles = new Vector3(0, 0, 0);
                    trafoUnk.localPosition = new Vector3(55, -40, 40);
                }
            }

            Entity servos = data.entities.Get("External services");
            if (!(servos is null))
            {
                Transform trafoUnk = servos.containers.GetCurrentGroup().transform;
                if (!(trafoUnk is null))
                {
                    trafoUnk.localEulerAngles = new Vector3(0, 0, 0);
                    trafoUnk.localPosition = new Vector3(20, -20, 60);
                }
            }
            ...
            try { data.VDE.startupScreen.SetActive(false); } catch { }
        }
        ...
        override internal Vector3 PositionShallowGroup(UI.Group.Container container)
        {
            ...
            toReturn = GetTier1ContainerPosition(container, GetRigidJointValueFromConf(UI.Joint.Type.MemberGroot, "JointMaxDistance") * 3);
            if (!shapesInRow.Contains(container))
            {
                shapesInRow.Add(container);
            }
        ...
        override internal Container.PositionCorrectionDirection GetPositionCorrectionDirection(int depthOfRelations)
        {
