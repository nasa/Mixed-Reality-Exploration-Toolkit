...
namespace GOV.NASA.GSFC.XR.MRET.Integrations.FortyTwo
{
    public class FortyTwoMenuController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                if (axes[0].activeSelf) // check if instrument is active
                    posNAxis(); //positions active instrument

                if (axes[1].activeSelf)
                    posLAxis();

                if (axes[2].activeSelf)
                    posFAxis();

                if (axes[3].activeSelf)
                    posBAxis();

                if (vectors[0].activeSelf)
                    posTruthVector(0);

                if (vectors[1].activeSelf)
                    posTruthVector(1);

                if (vectors[2].activeSelf)
                    posTruthVector(2);
