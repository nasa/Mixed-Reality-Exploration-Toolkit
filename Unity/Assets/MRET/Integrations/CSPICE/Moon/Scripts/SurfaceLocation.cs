...
using GOV.NASA.GSFC.XR.Utilities.Terrain;
...
namespace GOV.NASA.GSFC.XR.MRET.Integrations.CSPICE.LunarModel
{
    ...
    public class SurfaceLocation : MRETUpdateBehaviour
    {
        ...
        // Prefix of all keys
        public const string KEY_PREFIX = "GOV.NASA.GSFC.XR.SOLARSYSTEM.CELESTIALBODIES.LUNAR.MODEL.SURFACELOCATION";
        ...
        protected void UpdateUserTelemetry()
        {
            ...
            if (dataManager != null)
            {
                ...
                var userPosition = dataManager.FindPoint(User.KEY_USER_POSITION);
                if (userPosition is Vector3)
                {
                    ...
                    var userEyePosition = dataManager.FindPoint(User.KEY_USER_EYE_POSITION);
                ...
                var userOrientation = dataManager.FindPoint(User.KEY_USER_ORIENTATION);
        ...
        protected override void MRETStart()
        {
            ...
            //Log("Upper Left:  [" + Coordinates.ToDegrees("3d22'43.43E") + ", " + Coordinates.ToDegrees("26d 9'35.54N") + "]");
            //Log("Lower Right: [" + Coordinates.ToDegrees("3d45'16.02E") + ", " + Coordinates.ToDegrees("25d49'19.85N") + "]");
            //Log("Center: [" + Coordinates.ToDegrees("3d33'59.72E") + ", " + Coordinates.ToDegrees("25d59'27.69N") + "]");
            ...
            {
                LogError("Upper left terrain longitude could not be parsed: " + terrainUpperLeftLongitudeDms);
                errorEncountered = true;
            }
            terrainUpperLeftLatitude = Coordinates.ToDegrees(terrainUpperLeftLatitudeDms);
            if (double.IsNaN(terrainUpperLeftLatitude))
            {
                LogError("Upper left terrain latitude could not be parsed: " + terrainUpperLeftLatitudeDms);
                errorEncountered = true;
            }
            // Lower left
            ...
            {
                LogError("Lower left terrain longitude could not be parsed: " + terrainLowerLeftLongitudeDms);
                errorEncountered = true;
            }
            terrainLowerLeftLatitude = Coordinates.ToDegrees(terrainLowerLeftLatitudeDms);
            if (double.IsNaN(terrainLowerLeftLatitude))
            {
                LogError("Lower left terrain latitude could not be parsed: " + terrainLowerLeftLatitudeDms);
                errorEncountered = true;
            }
            // Upper right
            ...
            {
                LogError("Upper right terrain longitude could not be parsed: " + terrainUpperRightLongitudeDms);
                errorEncountered = true;
            }
            terrainUpperRightLatitude = Coordinates.ToDegrees(terrainUpperRightLatitudeDms);
            if (double.IsNaN(terrainUpperRightLatitude))
            {
                LogError("Upper right terrain latitude could not be parsed: " + terrainUpperRightLatitudeDms);
                errorEncountered = true;
            }
            // Lower right
            ...
            {
                LogError("Lower right terrain longitude could not be parsed: " + terrainLowerRightLongitudeDms);
                errorEncountered = true;
            }
            terrainLowerRightLatitude = Coordinates.ToDegrees(terrainLowerRightLatitudeDms);
            if (double.IsNaN(terrainLowerRightLatitude))
            {
                LogError("Lower right terrain latitude could not be parsed: " + terrainLowerRightLatitudeDms);
                errorEncountered = true;
            }
            // Center
            ...
            {
                LogError("Center terrain longitude could not be parsed: " + terrainCenterLongitudeDms);
                errorEncountered = true;
            }
            terrainCenterLatitude = Coordinates.ToDegrees(terrainCenterLatitudeDms);
            if (double.IsNaN(terrainCenterLatitude))
            {
                LogError("Center terrain latitude could not be parsed: " + terrainCenterLatitudeDms);
                errorEncountered = true;
            }
            ...
            if (errorEncountered)
            {
                LogError("Errors encountered during startup. Processing will not occur.");
            }
