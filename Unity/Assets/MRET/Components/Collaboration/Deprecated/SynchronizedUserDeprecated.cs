...
{
    [System.Obsolete("Refer to " + nameof(GOV.NASA.GSFC.XR.MRET.Collaboration.SynchronizedUser))]
    public class SynchronizedUserDeprecated : MonoBehaviour
    {
        ...
        public SynchronizedControllerDeprecated leftController, rightController;
        ...
        public static SynchronizedUserDeprecated Create(string alias, string avatar, UserType type, string color, string uuid = null,
            ...
            float sclRes = 0.05f)
        {
            ...
            if (cMan == null)
            {
                Debug.LogError("[SynchronizedUserDeprecated->Create] No Collaboration Manager found. Unable to produce a user.");
            }
        ...
        private static SynchronizedUserDeprecated CreateAvatar(GameObject avatarPrefab, Transform avatarParent,
            ...
            bool controlled = false, int throttle = 10, float transRes = 0.01f, float rotRes = 10f, float sclRes = 0.05f)
        {
            if (!controlled)
            {
                ...
                newUser.leftController = newUser.transform.Find("Left").gameObject.AddComponent<SynchronizedControllerDeprecated>();
                newUser.leftController.controllerSide = SynchronizedControllerDeprecated.ControllerSide.Left;
                ...
                newUser.leftController.pointer = newUser.transform.Find("Left/Laser").gameObject.AddComponent<SynchronizedPointerDeprecated>();
                ...
                {
                    newUser.rightController = newUser.transform.Find("Right").gameObject.AddComponent<SynchronizedControllerDeprecated>();
                    newUser.rightController.controllerSide = SynchronizedControllerDeprecated.ControllerSide.Right;
                    ...
                    newUser.rightController.pointer = newUser.transform.Find("Right/Laser").gameObject.AddComponent<SynchronizedPointerDeprecated>();
                    ...
                    if (transform.hasChanged && synchronizationManager != null)
                    {
                        ...
                        //if (currPos != lastRecordedPosition && positionThrottleCounter >= throttleFactor)
                        {
                            ...
                            //if ((currPos - lastRecordedPosition).magnitude > translationResolution)
                            {
                                ...
                                {
                                    XRCUnityDeprecated.UpdateEntityPosition(userAlias, XRCManager.USERCATEGORY, lastRecordedPosition,
                                        uuid.ToString(), null, GOV.NASA.GSFC.XR.XRC.UnitType.meter);
                                }
                                else
                                {
                        ...
                        //if (transform.localRotation != lastRecordedRotation && rotationThrottleCounter >= throttleFactor)
                        {
                            ...
                            //if ((transform.localRotation.eulerAngles - lastRecordedRotation.eulerAngles).magnitude > rotationResolution)
                            {
                                ...
                                if (collaborationManager.engineType == CollaborationManager.EngineType.XRC)
                                {
                                    XRCUnityDeprecated.UpdateEntityRotation(userAlias, XRCManager.USERCATEGORY, lastRecordedRotation,
                                        uuid.ToString(), null, GOV.NASA.GSFC.XR.XRC.UnitType.degrees);
                                }
                                else
                                {
                        ...
                        if (transform.localScale != lastRecordedScale && scaleThrottleCounter >= throttleFactor)
                        {
                            if ((transform.localScale - lastRecordedScale).magnitude > scaleResolution)
                            {
                                ...
                                if (collaborationManager.engineType == CollaborationManager.EngineType.XRC)
                                {
                                    XRCUnityDeprecated.UpdateEntityScale(userAlias, XRCManager.USERCATEGORY, lastRecordedScale,
                                        uuid.ToString(), null, GOV.NASA.GSFC.XR.XRC.UnitType.unitless);
                                }
                                else
                                {
